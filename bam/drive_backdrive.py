...
for velocity in range(10):
    ...
    for motor_torque in torques:
        ...
        for external_torque in torques:
            frictions, _ = model.compute_frictions(
                motor_torque, -external_torque, velocity
            )
            ...
            if (
                external_torque_high is None
                and motor_torque + frictions < external_torque
            ):
    ...
    plt.plot(
        ...
        lows,
        color="tab:blue",
        label=r"$\tau_{drive}(\tau_m)$" if velocity == 0 else None,
        alpha=np.exp(-velocity / 2),
    )
    plt.plot(
        torques,
        highs,
        color="tab:red",
        label=r"$\tau_{backdrive}(\tau_m)$" if velocity == 0 else None,
        alpha=np.exp(-velocity / 2),
    )
