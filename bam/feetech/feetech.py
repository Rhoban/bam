...
def convert_steps_to_degrees(
    steps: int | np.ndarray, models: str | list[str]
) -> np.ndarray:
    """This function converts the step range to the degree range for indicating motors rotation.
    It assumes a motor achieves a full rotation by going from -180 degree position to +180.
    The motor resolution (e.g. 4096) corresponds to the number of steps needed to achieve a full rotation.
    """
    resolutions = [MODEL_RESOLUTION[model] for model in models]
    degrees = steps * 180 / np.array(resolutions) * 2
    return degrees


def convert_steps_to_radians(
    steps: int | np.ndarray, models: str | list[str]
) -> np.ndarray:
    """This function converts the step range to the radian range for indicating motors rotation.
    It assumes a motor achieves a full rotation by going from -pi radian position to +pi.
    The motor resolution (e.g. 4096) corresponds to the number of steps needed to achieve a full rotation.
    """
    deg = convert_steps_to_degrees(steps, models)
    return np.deg2rad(deg)


def convert_radians_to_steps(
    radians: float | np.ndarray, models: str | list[str]
) -> np.ndarray:
    """This function converts the radian range to the step range for indicating motors rotation.
    It assumes a motor achieves a full rotation by going from -pi radian position to +pi.
    The motor resolution (e.g. 4096) corresponds to the number of steps needed to achieve a full rotation.
    """
    deg = np.rad2deg(radians)
    return convert_degrees_to_steps(deg, models)
...
def configure(motor_bus):
    ...
    motor_bus.write("P_Coefficient", 32)
    # Set I_Coefficient and D_Coefficient to default value 0 and 32
    motor_bus.write("I_Coefficient", 0)
    motor_bus.write("D_Coefficient", 32)
